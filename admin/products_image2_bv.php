...
// Define page object
$products_image2_blobview = new cproducts_image2_blobview();
$Page =& $products_image2_blobview;

// Page init processing
$products_image2_blobview->Page_Init();

// Page main processing
$products_image2_blobview->Page_Main();
?>
<?php
$products_image2_blobview->Page_Terminate();
?>
<?php
...
// Page Class
//
class cproducts_image2_blobview {

	// Page ID
	var $PageID = 'blobview';

	// Page Object Name
	var $PageObjName = 'products_image2_blobview';
	...
	//  - open connection
	//
	function cproducts_image2_blobview() {
		global $conn;
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('products', 'x_image2');
		...
		if ($rs = $conn->Execute($sSql)) {
			if (!$rs->EOF) {
				...
				if (trim(strval($rs->fields('image2'))) <> "") {
					header("Content-Disposition: attachment; filename=" . $rs->fields('image2'));
				}
				$objBinary->Value = $rs->fields('image2');
				if ($bShowThumbnail) {
