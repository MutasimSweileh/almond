...
// Define page object
$info_media_update = new cinfo_media_update();
$Page =& $info_media_update;

// Page init processing
$info_media_update->Page_Init();

// Page main processing
$info_media_update->Page_Main();
?>
...
// Create page object
var info_media_update = new ew_Page("info_media_update");

// page properties
info_media_update.PageID = "update"; // page ID
var EW_PAGE_ID = info_media_update.PageID; // for backward compatibility

// extend page with ValidateForm function
info_media_update.ValidateForm = function(fobj) {
...
info_media_update.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
info_media_update.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
info_media_update.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
info_media_update.ValidateRequired = false; // no JavaScript validation
<?php } ?>

// search highlight properties
info_media_update.ShowHighlightText = "Show highlight"; 
info_media_update.HideHighlightText = "Hide highlight";
...
<p><span class="phpmaker">Update TABLE: info media<br><br>
<a href="<?php echo $info_media->getReturnUrl() ?>">Back to List</a></span></p>
<?php $info_media_update->ShowMessage() ?>
<form name="finfo_mediaupdate" id="finfo_mediaupdate" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return info_media_update.ValidateForm(this);">
<p>
<input type="hidden" name="t" id="t" value="info_media">
<input type="hidden" name="a_update" id="a_update" value="U">
<?php for ($i = 0; $i < $info_media_update->nKeySelected; $i++) { ?>
<input type="hidden" name="k<?php echo $i+1 ?>_key" id="key<?php echo $i+1 ?>" value="<?php echo ew_HtmlEncode($info_media_update->arRecKeys[$i]) ?>">
<?php } ?>
...
<input type="checkbox" name="u_code" id="u_code" value="1"<?php echo ($info_media->code->MultiUpdate == "1") ? " checked=\"checked\"" : "" ?>>
</td>
		<td<?php echo $info_media->code->CellAttributes() ?>>code</td>
...
<input type="checkbox" name="u_link" id="u_link" value="1"<?php echo ($info_media->link->MultiUpdate == "1") ? " checked=\"checked\"" : "" ?>>
</td>
		<td<?php echo $info_media->link->CellAttributes() ?>>link</td>
...
<?php
$info_media_update->Page_Terminate();
?>
<?php
...
class cinfo_media_update {
	...
	// Page Object Name
	var $PageObjName = 'info_media_update';
	...
	function Page_Main() {
		...
		if ($this->nKeySelected == 0) {
			...
			if (@$_POST["a_update"] <> "") {
				...
				// Get action
				$info_media->CurrentAction = $_POST["a_update"];
		...
		if ($this->nKeySelected <= 0)
			$this->Page_Terminate("info_medialist.php"); // No records selected, return to list
	...
	function LoadMultiUpdateValues() {
		...
		while (!$rs->EOF) {
			if ($i == 1) {
				...
				if (!ew_CompareValue($info_media->code->DbValue, $rs->fields('code')))
					$info_media->code->CurrentValue = NULL;
				if (!ew_CompareValue($info_media->link->DbValue, $rs->fields('link')))
					$info_media->link->CurrentValue = NULL;
			}
			$i++;
			$rs->MoveNext();
		}
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$info_media->id->CurrentValue = $sKeyFld;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$info_media->code->MultiUpdate = $objForm->GetValue("u_code");
		$info_media->link->setFormValue($objForm->GetValue("x_link"));
		$info_media->link->MultiUpdate = $objForm->GetValue("u_link");
	...
	function ValidateForm() {
		...
		$gsFormError = "";
		$lUpdateCnt = 0;
		if ($info_media->code->MultiUpdate == "1") $lUpdateCnt++;
		if ($info_media->link->MultiUpdate == "1") $lUpdateCnt++;
		if ($lUpdateCnt == 0) {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if ($info_media->code->MultiUpdate <> "" && $info_media->code->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= "Please enter required field - code";
		}
		if ($info_media->link->MultiUpdate <> "" && $info_media->link->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
