...
// Define page object
$products_file_blobview = new cproducts_file_blobview();
$Page =& $products_file_blobview;

// Page init processing
$products_file_blobview->Page_Init();

// Page main processing
$products_file_blobview->Page_Main();
?>
<?php
$products_file_blobview->Page_Terminate();
?>
<?php
...
// Page Class
//
class cproducts_file_blobview {

	// Page ID
	var $PageID = 'blobview';

	// Page Object Name
	var $PageObjName = 'products_file_blobview';
	...
	//  - open connection
	//
	function cproducts_file_blobview() {
		global $conn;
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('products', 'x_file');
		...
		if ($rs = $conn->Execute($sSql)) {
			if (!$rs->EOF) {
				...
				if (trim(strval($rs->fields('file'))) <> "") {
					header("Content-Disposition: attachment; filename=" . $rs->fields('file'));
				}
				$objBinary->Value = $rs->fields('file');
				if ($bShowThumbnail) {
