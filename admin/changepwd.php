...
class cchangepwd {
	...
	//
	function Page_Main() {
		global $conn, $Security, $gsFormError, $users;
		if (!ew_IsHttpPost())
			return;
		$bPwdUpdated = FALSE;
			...
			if ($rs = $conn->Execute($sSql)) {
				if (!$rs->EOF) {
					if ((EW_MD5_PASSWORD && md5($sOPwd) == $rs->fields('password')) ||
						...
						$bValidPwd = $conn->Execute($users->UpdateSQL($rsnew));
						$conn->raiseErrorFn = '';
						if ($bValidPwd)
							$bPwdUpdated = TRUE;
					}	else {
		...
		if ($bPwdUpdated) {
			...
			$this->setMessage("Password Changed"); // set up message
	...
	function ValidateForm($opwd, $npwd, $cpwd) {
		...
		if ($opwd == "") {
			...
			$gsFormError .= "Please enter old password";
		}
		if ($npwd == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= "Please enter new password";
		}
		if ($npwd <> $cpwd) {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
