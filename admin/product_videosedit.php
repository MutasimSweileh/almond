...
// Define page object
$product_videos_edit = new cproduct_videos_edit();
$Page =& $product_videos_edit;

// Page init processing
$product_videos_edit->Page_Init();

// Page main processing
$product_videos_edit->Page_Main();
?>
...
// Create page object
var product_videos_edit = new ew_Page("product_videos_edit");

// page properties
product_videos_edit.PageID = "edit"; // page ID
var EW_PAGE_ID = product_videos_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
product_videos_edit.ValidateForm = function(fobj) {
...
product_videos_edit.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
product_videos_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
product_videos_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
product_videos_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>

// search highlight properties
product_videos_edit.ShowHighlightText = "Show highlight"; 
product_videos_edit.HideHighlightText = "Hide highlight";
...
<p><span class="phpmaker">Edit TABLE: product videos<br><br>
<a href="<?php echo $product_videos->getReturnUrl() ?>">Go Back</a></span></p>
<?php $product_videos_edit->ShowMessage() ?>
<form name="fproduct_videosedit" id="fproduct_videosedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return product_videos_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="product_videos">
...
<?php if ($product_videos->id->Visible) { // id ?>
	<tr<?php echo $product_videos->id->RowAttributes ?>>
		...
		<td<?php echo $product_videos->id->CellAttributes() ?>><span id="el_id">
<div<?php echo $product_videos->id->ViewAttributes() ?>><?php echo $product_videos->id->EditValue ?></div><input type="hidden" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($product_videos->id->CurrentValue) ?>">
</span><?php echo $product_videos->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<?php
$product_videos_edit->Page_Terminate();
?>
<?php
...
class cproduct_videos_edit {
	...
	// Page Object Name
	var $PageObjName = 'product_videos_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$product_videos->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate Form
			if (!$this->ValidateForm()) {
				$product_videos->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$this->RestoreFormValues();
			}
		} else {
			$product_videos->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($product_videos->id->CurrentValue == "")
			$this->Page_Terminate("product_videoslist.php"); // Invalid key, return to list
		...
		// Render the record
		$product_videos->RowType = EW_ROWTYPE_EDIT; // Render as edit
		$this->RenderRow();
	}
