...
// Define page object
$products_image3_blobview = new cproducts_image3_blobview();
$Page =& $products_image3_blobview;

// Page init processing
$products_image3_blobview->Page_Init();

// Page main processing
$products_image3_blobview->Page_Main();
?>
<?php
$products_image3_blobview->Page_Terminate();
?>
<?php
...
// Page Class
//
class cproducts_image3_blobview {

	// Page ID
	var $PageID = 'blobview';

	// Page Object Name
	var $PageObjName = 'products_image3_blobview';
	...
	//  - open connection
	//
	function cproducts_image3_blobview() {
		global $conn;
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('products', 'x_image3');
		...
		if ($rs = $conn->Execute($sSql)) {
			if (!$rs->EOF) {
				...
				if (trim(strval($rs->fields('image3'))) <> "") {
					header("Content-Disposition: attachment; filename=" . $rs->fields('image3'));
				}
				$objBinary->Value = $rs->fields('image3');
				if ($bShowThumbnail) {
