...
// PHPMaker 6 configuration for Table gallery
$gallery = NULL; // Initialize table object
...
class cgallery {
	...
	function cgallery() {
		$this->TableVar = "gallery";
		$this->TableName = "gallery";
		$this->SelectLimit = TRUE;
		$this->id = new cField('gallery', 'x_id', 'id', "`id`", 3, -1, FALSE);
		$this->fields['id'] =& $this->id;
		$this->name = new cField('gallery', 'x_name', 'name', "`name`", 200, -1, FALSE);
		$this->fields['name'] =& $this->name;
		$this->name_arabic = new cField('gallery', 'x_name_arabic', 'name_arabic', "`name_arabic`", 200, -1, FALSE);
		$this->fields['name_arabic'] =& $this->name_arabic;
		$this->image = new cField('gallery', 'x_image', 'image', "`image`", 200, -1, TRUE);
		$this->fields['image'] =& $this->image;
		$this->category = new cField('gallery', 'x_category', 'category', "`category`", 3, -1, FALSE);
		$this->fields['category'] =& $this->category;
		$this->link = new cField('gallery', 'x_link', 'link', "`link`", 200, -1, FALSE);
		$this->fields['link'] =& $this->link;
		$this->special = new cField('gallery', 'x_special', 'special', "`special`", 3, -1, FALSE);
		$this->fields['special'] =& $this->special;
		$this->order = new cField('gallery', 'x_order', 'order', "`order`", 3, -1, FALSE);
		$this->fields['order'] =& $this->order;
		$this->active = new cField('gallery', 'x_active', 'active', "`active`", 3, -1, FALSE);
	...
	// Master filter
	function SqlMasterFilter_categories() {
		return "`id`=@id@";
	}

	// Detail filter
	function SqlDetailFilter_categories() {
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `gallery` ($names) VALUES ($values)";
	}
	...
	// View url
	function ViewUrl() {
		return $this->KeyUrl("galleryview.php", $this->UrlParm());
	}

	// Add url
	function AddUrl() {
		$AddUrl = "galleryadd.php";
	...
	// Edit url
	function EditUrl() {
		return $this->KeyUrl("galleryedit.php", $this->UrlParm());
	}
	...
	// Copy url
	function CopyUrl() {
		return $this->KeyUrl("galleryadd.php", $this->UrlParm());
	}
	...
	// Delete url
	function DeleteUrl() {
		return $this->KeyUrl("gallerydelete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=gallery" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function RenderListRow() {
		...
		if (strval($this->category->CurrentValue) <> "") {
			$sSqlWrk = "SELECT `name`, `name_arabic` FROM `categories` WHERE `id` = " . ew_AdjustSql($this->category->CurrentValue) . "";
			$rswrk = $conn->Execute($sSqlWrk);
			if ($rswrk && !$rswrk->EOF) { // Lookup value(s) found
				$this->category->ViewValue = $rswrk->fields('name');
				$this->category->ViewValue .= ew_ValueSeparator(0) . $rswrk->fields('name_arabic');
				$rswrk->Close();
			} else {
